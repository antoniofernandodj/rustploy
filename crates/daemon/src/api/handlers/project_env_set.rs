...
use shared::{EnvVar, Response as RpResponse};
use tracing::info;

pub async fn handle(state: AppState, project_id: String, env_vars: Vec<EnvVar>) -> RpResponse {
    info!(
        project_id = %project_id,
        count = env_vars.len(),
        "project_env_set: atualizando env vars do projeto"
    );
    match crate::db::projects::update_env_vars(&state.db, &project_id, env_vars).await {
        Ok(Some(project)) => {
