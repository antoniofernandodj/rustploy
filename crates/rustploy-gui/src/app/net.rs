...
/// Re-fetches the project/service inventory on `conn` and returns the
/// Projects-grid context keys (`projects`/`project_rows` + counts), filtered by
/// `search`. Lets project create/delete patch the grid immediately instead of
...
async fn projects_grid_pairs(conn: &mut rwp::RwpStream, search: &str) -> Vec<(String, String)> {
    ...
    vec![
        ...
        ("projects".into(), projects_json(&projects_raw, &all, &term)),
        ("project_rows".into(), project_rows_json(&projects_raw, &all, &term)),
    ]
}

/// Creates a project (Projects tab "Novo projeto" bar) and immediately re-fetches
/// the Projects grid on the same connection so the new card shows up at once,
...
/// still has services, so no client-side re-validation is needed here. Re-fetches
...
/// on the single service open in the detail view). Re-fetches the project's
