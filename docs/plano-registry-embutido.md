...
>
> **Fase 3 (integração com o deploy executor) implementada em 2026-07-13** — a Fase 2 tinha quebrado silenciosamente o pull de imagens do próprio registry embutido pelo deploy executor (401, porque o Docker Engine do host não mandava credencial nenhuma). Corrigido com um token interno `rp-internal`, regenerado a cada boot do daemon (só o hash SHA-256 persiste, via `registry_tokens::upsert_internal` com `ON CONFLICT(name)`; texto plano só existe na memória do processo — `crates/daemon/src/registry/internal_token.rs::ensure`), gerado em `main.rs` logo após conectar o banco (antes de `deploy::recovery::recover`) e roteado por `Option<Arc<str>>` até todo lugar que cria um `DeployExecutor` (`AppState`, `deploy/recovery.rs`, `api/deployments.rs`, `api/handlers/deploy_start.rs`, `watchdog.rs`). `docker/images.rs::pull()` ganhou parâmetro `credentials: Option<DockerCredentials>` (repassado ao `create_image`); `DeployExecutor::registry_credentials_for` detecta se a imagem do serviço aponta pro registry embutido — por `127.0.0.1:<porta>`/`localhost:<porta>` (pull no mesmo host, caso comum) ou pelo domínio público configurado sem porta (acesso externo via ingress) — via a função pura `is_embedded_registry_image` (testada isoladamente). `registry_tokens::list()` filtra a linha `rp-internal` (não é gerenciável pela UI) e `RegistryTokenCreate` rejeita esse nome reservado. Validado com `cargo test -p daemon` (57 testes) + `cargo check --workspace` limpos; ver seção 7 do plano original de sessão para o roteiro de smoke test manual (não repetido nesta nota — depende de ambiente com registry habilitado).
