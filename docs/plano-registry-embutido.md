...
>
> **Fase 1 (núcleo push/pull) implementada em 2026-07-13** — `crates/daemon/src/registry/` (`name.rs`/`error.rs`/`storage.rs`/`http.rs`) + `crates/daemon/src/db/registry.rs` + bloco `[registry]` em `shared/src/config.rs`. Validada com `docker push`/`pull` reais (incluindo multi-arch via `buildx --push`, manifest index). Loopback only (`127.0.0.1:5100`), sem autenticação, sem ingress/TLS, sem integração com o deploy executor, sem GC — tudo isso continua Fase 2/3/4, não implementado. Retomar a partir daqui quando for atacar a próxima fase.
>
